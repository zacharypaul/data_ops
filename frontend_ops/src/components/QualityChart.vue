...
const createChart = () => {
  ...
  const labels = props.chartData.map(item => item.table);
  const dataValues = props.chartData.map(item => item.passRate);
  const backgroundColors = dataValues.map(value => {
    if (value >= 90) return 'rgba(16, 185, 129, 0.7)'; // Green for high pass rate
    if (value >= 70) return 'rgba(245, 158, 11, 0.7)'; // Yellow for medium pass rate
    return 'rgba(239, 68, 68, 0.7)'; // Red for low pass rate
  });
